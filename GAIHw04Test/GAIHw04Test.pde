...
static final int WHITE_DEPTH = 5;
static final int BLACK_DEPTH = 5;

private final String TABLES_FILE = dataPath("") + "/tables.txt";

float WIN_VAL = 100;

boolean gameOver = false;

static final boolean AIOnly = true;//Note: AIOnly requires a decent amount of memory, especially as search depth increases

static final boolean learning = false;

int[][] board;

int[] keyTable;

HashMap<Integer, MMXMove> mmxTable;
HashMap<Integer, MMXMove> mmxabTable;
HashMap<Integer, MMXMove> mtdabTable;
...
static final Algo BLACK_ALGO = Algo.MMXAB;
static final Algo WHITE_ALGO = Algo.MMXAB;
...
void readTables() {
  try {
    ...
    mmxTable = (HashMap<Integer, MMXMove>) o.readObject();
    mmxabTable = (HashMap<Integer, MMXMove>) o.readObject();
    mtdabTable = (HashMap<Integer, MMXMove>) o.readObject();
    o.close();
    f.close();
  } 
  catch(Exception e) {
...
void checkTableInit() {
  ...
  mmxTable = (mmxTable == null) ? new HashMap<Integer, MMXMove>() : mmxTable;
  mmxabTable = (mmxabTable == null) ? new HashMap<Integer, MMXMove>() : mmxabTable;
  mtdabTable = (mtdabTable == null) ? new HashMap<Integer, MMXMove>() : mtdabTable;
  doneRead = true;
}
...
void cleanup() {
  createOutput(TABLES_FILE);
  try {
    FileOutputStream f = new FileOutputStream(TABLES_FILE);
    ObjectOutputStream o = new ObjectOutputStream(f);
    o.writeObject(keyTable);
    o.writeObject(mmxTable);
    o.writeObject(mmxabTable);
    o.writeObject(mtdabTable);
    o.close();
    f.close();
  } 
  catch(Exception e) {
...
void draw() {
  if (setup) {
    ...
    textAlign(CENTER);
    fill(255);
    text("Loading AI Tables", WIN_ANNOUNCE_X, WIN_ANNOUNCE_Y);
    if (doneRead) {
      reading = false;
      doneRead = false;
    }
...
static class MMXMove implements Serializable, Comparable {
  int row;
  int col;
  int winVal = 0;//# of wins - # of losses
  float value;
  int depth = 0;
  ;
  MMXMove() {
...
void AIPlay(int[][] board, boolean whiteTurn, ArrayList<Move> legalMoves) {
  ...
  Algo AI_ALGO = whiteTurn ? WHITE_ALGO : BLACK_ALGO;
  int MAX_DEPTH = whiteTurn ? WHITE_DEPTH : BLACK_DEPTH;
  if (AI_ALGO == Algo.MMX) {
    bestMove = minimax(board, 0, whiteTurn, MAX_DEPTH);
  } else if (AI_ALGO == Algo.MMXAB) {
    bestMove = minimaxAB(board, MIN_FLOAT, MAX_FLOAT, 0, whiteTurn, MAX_DEPTH, false);
  } else if (AI_ALGO == Algo.MTD) {
    bestMove = mtdAlgo(board, 0, whiteTurn, MAX_DEPTH);
  } else {
    bestMove = new MMXMove(legalMoves.get(0).row, legalMoves.get(0).col, 0);
  }
...
MMXMove minimax(int[][] board, int currentDepth, boolean white, int maxDepth) {
  ArrayList<Move> legalMoves = generateLegalMoves(board, white);
  Integer hash = hashBoard(board, white);
  MMXMove fromTable = getFromTable(mmxTable, hash, maxDepth);
  if (fromTable != null) { 
    return fromTable;
  }

  if (currentDepth >= maxDepth || legalMoves.isEmpty()) {
    MMXMove last = new MMXMove(evaluationFunction(board));
    last.depth = maxDepth;
    putIntoTable(last, mmxTable, hash);
  }

  MMXMove best = !white ? new MMXMove(MIN_FLOAT) : new MMXMove(MAX_FLOAT);
  for (Move m : legalMoves) {
    int[][] boardCpy = fakeMove(board, m, white);
    MMXMove tmp = minimax(boardCpy, currentDepth + 1, !white, maxDepth);
  ...
  best.depth = maxDepth;
  putIntoTable(best, mmxTable, hash);
  return best;
}

//Based on minimax pseudocode in the Lecture Slides
MMXMove minimaxAB(int[][] board, float alpha, float beta, int currentDepth, boolean white, int maxDepth, boolean fromMTD) {
  ArrayList<Move> legalMoves = generateLegalMoves(board, white);
  Integer hash = hashBoard(board, white);
  MMXMove fromTable = fromMTD ? getFromTable(mtdabTable, hash, maxDepth) : getFromTable(mmxabTable, hash, maxDepth);
  ...
  if (currentDepth >= maxDepth || legalMoves.isEmpty()) {
    ...
    last.depth = maxDepth;
    if (!fromMTD) {
      putIntoTable(last, mmxabTable, hash);
    } else if (fromMTD) {
      putIntoTable(last, mtdabTable, hash);
    }
    return last;
  }
  ...
  for (Move m : legalMoves) {
    int[][] boardCpy = fakeMove(board, m, white);
    MMXMove tmp = minimaxAB(boardCpy, alpha, beta, currentDepth + 1, !white, maxDepth, fromMTD);
    if (!white && tmp.value >= best.value) {//MAX
      best = new MMXMove(m.row, m.col, tmp.value);
  ...
  best.depth = maxDepth;
  if (!fromMTD) {
    putIntoTable(best, mmxabTable, hash);
  } else if (fromMTD) {
    putIntoTable(best, mtdabTable, hash);
  }
  return best;
}
...
MMXMove mtdAlgo(int[][]board, float f, boolean white, int maxDepth) {
  ...
  while (lowBound < upBound) {
    ...
    best = minimaxAB(board, beta - 1, beta, 0, white, maxDepth, true);
    if (best.value < beta) {
      upBound = best.value;
    } else {
      lowBound = best.value;
...
void putIntoTable(MMXMove m, HashMap<Integer, MMXMove> table, Integer hash) {
  if (!table.containsKey(hash) || abs(table.get(hash).value) < abs(m.value) || table.get(hash).depth < m.depth) {
    table.put(hash, m);
  }
}

MMXMove getFromTable(HashMap<Integer, MMXMove> table, Integer hash, int maxDepth) {
  if (table.containsKey(hash) && table.get(hash).depth >= maxDepth) {
